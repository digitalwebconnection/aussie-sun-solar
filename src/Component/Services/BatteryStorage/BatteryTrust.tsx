...
const BatteryTrust = () => {
  const deliveries = [
    { title: 'Licensed Battery Installers', desc: 'Every installation is completed by certified professionals ensuring safety and long-term reliability.', icon: Hammer, gradient: 'from-orange-400 to-orange-500' },
    { title: 'Premium Tier-1 Batteries', desc: 'We use high-performance battery systems designed for maximum efficiency, durability, and savings.', icon: ShieldCheck, gradient: 'from-blue-500 to-cyan-400' },
    { title: 'CEC Approved Systems', desc: 'All systems include Clean Energy Council approved components for trusted performance and eligibility.', icon: Zap, gradient: 'from-purple-500 to-pink-500' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-14">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="inline-flex items-center gap-2 px-10 py-2 rounded-full bg-[#FE9900]/10 text-black border border-[#FE9900]/80 font-bold uppercase tracking-[3px] text-xs mb-6">
            <Sparkles size={15} /> Trusted Battery Installation
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {deliveries.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: i * 0.15 }} whileHover={{ y: -14 }} className="group relative overflow-hidden rounded-lg border border-gray-300 bg-white p-5 md:p-10 shadow-[0_10px_40px_rgba(0,0,0,0.24)] transition-all duration-500 hover:shadow-[0_20px_60px_rgba(0,0,0,0.8)]">
              ...
              <h3 className="relative z-10 text-3xl font-black text-[#0B1220] mb-6 leading-tight">{item.title}</h3>
              <p className="relative z-10 text-gray-900 text-lg leading-relaxed">{item.desc}</p>
