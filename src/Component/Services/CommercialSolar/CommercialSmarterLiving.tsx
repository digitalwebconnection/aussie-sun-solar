...
const CommercialSmarterLiving = () => {
  ...
  return (
    <section id="commercialsolar-smarter-living" aria-label="CommercialSolar Smart Living Features" className="relative overflow-hidden bg-white py-10 md:py-24">
        ...
        <div className="grid lg:grid-cols-2 gap-8 md:gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -80 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="relative">
            <div className="relative rounded-lg overflow-hidden shadow-[0_25px_80px_rgba(0,0,0,0.15)]">
              <img src={solarRoof1} alt="Commercial corporate building rooftop solar arrays" className="w-full h-[260px] sm:h-[380px] md:h-[550px] object-cover hover:scale-105 transition-transform duration-700" />
              ...
              <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} viewport={{ once: true }} className="absolute bottom-3 left-3 sm:bottom-6 sm:left-6 bg-white/95 backdrop-blur-md p-3 sm:p-6 rounded-lg shadow-xl max-w-[180px] sm:max-w-xs">
                <h3 className="text-lg sm:text-xl md:text-2xl font-serif font-semibold text-[#FE9900] leading-none">500+</h3>
                <p className="text-[#004093] font-bold mt-1 sm:mt-2 text-xs sm:text-sm">Commercial Solar Systems Across Australia</p>
              </motion.div>
            </div>
