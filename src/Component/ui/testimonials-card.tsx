      ...
      >
          ...
          <AnimatePresence custom={direction}>
            {items.map((item, index) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className="absolute inset-0 w-full h-full overflow-hidden border-[6px] bg-neutral-200 border-white shadow-2xl rounded-lg"
                  initial={{
                    x: offset * 15,
                    y: Math.abs(offset) * 6,
        ...
        <div className="col-start-2 row-start-2 flex flex-col justify-center min-h-[120px]">
          <AnimatePresence mode="wait">
            ...
            >
              <h3 className="text-2xl font-bold text-black">{activeItem.title}</h3>
              <p className="textlg text-neutral-600  mt-2">
        ...
        {showNavigation && items.length > 1 && (
          <div className="col-start-2 row-start-3 flex gap-2 mt-4">
            <button
              ...
              onClick={handlePrev}
              className={cn(
                "flex items-center justify-center w-10 h-10 rounded-full border border-neutral-200 bg-white transition-all",
                activeIndex === 0
